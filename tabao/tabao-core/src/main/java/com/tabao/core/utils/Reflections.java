...
public class Reflections {
	...
	public static Object getFieldValueByName(String fieldName, Object o) {
		try {
			...
			String getter = GETTER_PREFIX + firstLetter + fieldName.substring(1);
	...
	/**
	 * 获取属性类型(type)，属性名(name)，属性值不为空的字段(value)的map组成的list existAn 是否获取非映射字典的属性值 true
	 * 获取，false，不取
	 */
	public static List<Map<String, Object>> getFiledsInfo(Object o, boolean existAn) {
		Field[] fields = o.getClass().getDeclaredFields();
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (!fields[i].isAnnotationPresent(Transient.class)
					|| fields[i].isAnnotationPresent(Transient.class) == existAn) {
				if (getFieldValueByName(fields[i].getName(), o) != null
						&& !getFieldValueByName(fields[i].getName(), o).equals("")) {
